...
def move(frame,img):
	...
	if landing == False:
		...
		if cx > 160 and cx < 480 and cy > 90 and cy < 270 and area > 5000:
			...
			# 		prevCX = 320 - prevCX
			# 		prevCY = 160 - prevCY
			# 		fx = -0.025*prevCX/320.0
			# 		fy = 0.025*prevCY/160.0
			# 		drone.move(-fx,fy,0.01,0.0)
			# 	elif lastKnownQuad == 2:
			# 		prevCX = prevCX - 320
			# 		prevCY = 160 - prevCY
			# 		fx = 0.025*prevCX/320.0
			# 		fy = 0.025*prevCY/160.0
			# 		drone.move(fx,fy,0.01,0.0)
			# 	elif lastKnownQuad == 3:
			# 		prevCX = 320-prevCX
			# 		prevCY = prevCY - 160
			# 		fx = -0.025*prevCX/320.0
			# 		fy = -0.025*prevCY/160.0
			# 		drone.move(fx,fy,0.01,0.0)
			# 	elif lastKnownQuad == 4:
			# 		prevCX = prevCX - 320
			# 		prevCY = prevCY - 160
			# 		fx = 0.025*prevCX/320.0
			# 		fy = -0.025*prevCY/160.0
...
def find_contours(frame,img):
	...
	lower_range = np.array([150, 25, 25], dtype=np.uint8)
